...
from .math import godel_composition, soft_godel_composition


class DefeasibleReasoning:
    """
    Defeasible relation closure with positive and negative arrows.

    Positive rules are closed transitively. Negative rules coming from facts
    reachable from the start node down-weight conclusions they defeat.
    """
    ...
    def deliberate(self, iterations=3, start_node=0):
        """Apply active defeaters, then return the positive transitive closure."""
        filtered_pos = self.R_pos.clone()

        R_reach = self.R_pos.clone()
        for _ in range(iterations):
            R_reach = torch.max(R_reach, godel_composition(R_reach, R_reach))

        active_facts = R_reach[start_node, :]
        active_facts[start_node] = 1.0

        active_mask = (active_facts > 0).float().unsqueeze(1)
        active_R_neg = self.R_neg * active_mask
        defeater_multipliers = torch.prod(1.0 - active_R_neg, dim=0)
        filtered_pos = filtered_pos * defeater_multipliers.unsqueeze(0)

        R_current_pos = filtered_pos.clone()
        for _ in range(iterations):
            R_next_pos = godel_composition(R_current_pos, R_current_pos)
...
class AutonomousTheoremProver:
    """
    Smooth relation-composition demo for discovering implied edges.

    This is a heuristic theorem-discovery toy model, not a formal prover.
    """
    ...
    def discover_theorems(self, iterations=3, threshold=0.75):
        """Return the closed relation and newly discovered above-threshold edges."""
        new_theorems = []
